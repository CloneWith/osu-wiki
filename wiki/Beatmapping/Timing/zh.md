...
**测定时间轴 (Timing)** 又称为计时、测轴、测时，是通过测定歌曲每段[计时部分](/wiki/Client/Beatmap_editor/Timing)的位置、[每分钟节拍数 (BPM)](/wiki/Music_theory/Tempo)、[拍子记号](/wiki/Music_theory/Time_signature)，来确定歌曲音乐结构的过程。一组测定的时间轴也被称为 *Timing group* 或 *Timing set*。
...
歌曲文件的开头位置，与歌曲第一个下拍的位置之间的时间，称为总的*偏移量 (offset)*。它通常也是第一根红线的位置，osu! 会利用它来推算出其他节拍的位置。通常使用一根红线就能完美满足使用现代数字编辑或制作的歌曲 — 它们没有变奏和变拍，也没有散板<!-- TODO: link -->。
